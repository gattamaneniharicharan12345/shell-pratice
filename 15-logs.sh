...
Logs_Floder="/var/logs/shellscript-logs"
...
if [ $USERID -ne 0 ]
then
    echo -e "$RError:: please run this scripit with root access $N" | tee -a Log_file
    exit 1
else
...
then
    ...
    echo -e "Mysql is installed.... $Y nothing todo $N" | tee -a Log_file
fi  
...
then
    ...
    echo -e "nginx is installed... $Y nothing to do $N" | tee -a Log_file
fi
...
then
    ...
    echo -e "Python3 is installed.. $Y noyhing to do $N" | tee -a Log_file
fi    
